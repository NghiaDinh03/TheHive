...
const TABS = ['Details', 'Tasks', 'Observables', 'Logs', 'Attachments', 'Procedures', 'Shares', 'Audit'] as const;
...
export default function CaseDetailPage() {
  ...
  const addCF = useMutation({ mutationFn: () => apiFetch(`/api/v1/cases/${params.id}/custom-fields`, { method: 'POST', json: cfForm }), onSuccess: () => { setCfForm({ name: '', value: '', field_type: 'string' }); refetch(); } });
          ...
          <section className="content case-page next-case-page">
            ...
            <div className="case-detail-layout">
              ...
              <div className="tab-content case-page-content">
                ...
                {item && activeTab === 'Details' && <DetailsTab item={item} customFields={detail.data?.custom_fields ?? []} cfForm={cfForm} setCfForm={setCfForm} addCF={addCF} deleteCF={deleteCF} updateCase={updateCase} canWrite={canWrite} />}
            ...
            <aside className="box box-primary case-action-box">
              ...
              <div className="box-body detail-side-list">
                ...
                <button className="thehive-btn-secondary danger" disabled={!canWrite || deleteCase.isPending} onClick={() => deleteCase.mutate()}>Delete</button>
              </div>
...
function DetailsTab({ item, customFields, cfForm, setCfForm, addCF, deleteCF, updateCase, canWrite }: {
  ...
  addCF: { mutate: () => void; isPending: boolean }; deleteCF: { mutate: (id: string) => void };
  ...
  return (<>
    ...
    <div className="row">
      <div className="col-xs-12">
        ...
        <table className="table table-striped case-custom-field-table"><thead><tr><th>Name</th><th>Value</th><th style={{ width: 90 }}></th></tr></thead><tbody>
          {customFields.map(cf => <tr key={cf.name}><td>{cf.name}</td><td>{cf.value}</td><td>{canWrite && cf.id && <button className="btn btn-xs btn-danger" onClick={() => deleteCF.mutate(cf.id!)}>Delete</button>}</td></tr>)}
          {!customFields.length && <tr><td colSpan={3} className="empty-message">No custom fields defined for this case.</td></tr>}
        </tbody></table>
        {canWrite && <div className="filter-panel" style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <label style={{ flex: 1 }}>Name<input className="form-control input-sm" value={cfForm.name} onChange={e => setCfForm((f: typeof cfForm) => ({ ...f, name: e.target.value }))} /></label>
          <label style={{ flex: 1 }}>Value<input className="form-control input-sm" value={cfForm.value} onChange={e => setCfForm((f: typeof cfForm) => ({ ...f, value: e.target.value }))} /></label>
          <button className="btn btn-sm btn-primary" disabled={!cfForm.name.trim() || addCF.isPending} onClick={() => addCF.mutate()}><i className="fa fa-plus" /> Add field</button>
        </div>}
      </div>
    </div>
...
function TasksTab({ tasks, caseId, canWrite, showTaskForm, setShowTaskForm, taskForm, setTaskForm, createTask, closeTask, assignTask }: {
  ...
  return (<>
    ...
    <table className="table table-hover valigned tasks-table data-list" style={{ marginTop: 10 }}><thead><tr><th></th><th>Group</th><th>Task</th><th>Status</th><th>Assignee</th><th>Date</th><th>Due/SLA</th><th>Order</th>{canWrite && <th className="text-right">Actions</th>}</tr></thead><tbody>
      {tasks.map(t => <tr key={t.id} className={t.flag ? 'task-flagged' : ''}>
        <td className="task-status" align="center"><TaskFlags task={t} /></td>
        <td>{t.group_name || '—'}</td>
        <td><a href={`/tasks/${t.id}`} className={t.flag ? 'text-danger' : ''}>{t.flag && <span className="text-danger noline mr-xxxs" title="Action Required"><i className="fa fa-exclamation-triangle" /></span>}{t.title}</a>{t.description && <div className="mt-xxs filter-panel markdown" style={{ fontSize: '0.8rem' }}>{t.description.slice(0, 120)}{t.description.length > 120 ? '...' : ''}</div>}</td>
        ...
        <td style={{ whiteSpace: 'nowrap' }}><button className="thehive-btn-sm" onClick={() => reorderTask(t, -1)}>↑</button><span style={{ margin: '0 6px' }}>{t.order_index}</span><button className="thehive-btn-sm" onClick={() => reorderTask(t, 1)}>↓</button></td>
  ...
  return (<>
    ...
    <table className="table table-hover valigned data-list observable-table" style={{ marginTop: 10 }}><thead><tr><th>TLP</th><th>Flags</th><th>Type</th><th>Data</th><th>Tags</th><th>Owner</th>{canWrite && <th>Actions</th>}</tr></thead><tbody>
      {observables.map(o => <tr key={o.id}>
        ...
        <td>{o.created_by}</td>
        {canWrite && <td><button className="thehive-btn-sm danger" onClick={() => deleteObs.mutate(o.id)}>Xoá</button></td>}
      </tr>)}
      {!observables.length && <tr><td colSpan={canWrite ? 7 : 6} className="thehive-empty">Chưa có observable.</td></tr>}
    </tbody></table>
  </>);
}
...
function LogsTab({ logs, history, logMessage, setLogMessage, appendLog, canWrite }: {
  ...
  return (<>
    {canWrite && <>
      <h3 className="detail-section-title">Thêm ghi chú</h3>
      <div className="detail-action-panel">
        <textarea className="thehive-input" rows={4} value={logMessage} onChange={e => setLogMessage(e.target.value)} placeholder="Viết ghi chú… (hỗ trợ markdown cơ bản: **bold**, *italic*, `code`, - list)" />
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          <input type="file" className="thehive-input" onChange={e => setFile(e.target.files?.[0] || null)} />
          <button className="thehive-btn-primary" disabled={!logMessage.trim() || appendLog.isPending} onClick={() => { appendLog.mutate(file); setFile(null); }}>
            {appendLog.isPending ? 'Đang lưu…' : 'Thêm ghi chú'}
          </button>
        </div>
      </div>
    </>}
    ...
    <div className="timeline">
      ...
      {!events.length && <div className="thehive-empty">Chưa có sự kiện nào.</div>}
    </div>
  </>);
}
...
function ProceduresTab({ procedures, procForm, setProcForm, addProc, deleteProc, canWrite }: {
  ...
  return (<>
    ...
    <table className="thehive-table"><thead><tr><th>Pattern</th><th>Tactic</th><th>Mô tả</th><th>Thời gian</th><th>Người tạo</th><th></th></tr></thead><tbody>
      {procedures.map(p => <tr key={p.id}><td>{p.pattern_name || p.pattern_id}</td><td>{p.tactic || '—'}</td><td>{p.description || '—'}</td><td>{p.occurred_at ? new Date(p.occurred_at).toLocaleString() : '—'}</td><td>{p.created_by}</td><td>{canWrite && <button className="thehive-btn-sm danger" onClick={() => deleteProc.mutate(p.id)}>Xoá</button>}</td></tr>)}
      {!procedures.length && <tr><td colSpan={6} className="thehive-empty">Chưa có procedure.</td></tr>}
    </tbody></table>
    {canWrite && <div className="detail-action-panel" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
      ...
      <label style={{ flex: 1 }}>Tên<input className="thehive-input" value={procForm.pattern_name} onChange={e => setProcForm((f: typeof procForm) => ({ ...f, pattern_name: e.target.value }))} /></label>
      ...
      <button className="thehive-btn-primary" disabled={!procForm.pattern_id.trim() || addProc.isPending} onClick={() => addProc.mutate()}>Thêm</button>
    </div>}
  </>);
}
...
function AuditTab({ history }: { history: History[] }) {
  return (<>
    <h3 className="detail-section-title">Lịch sử</h3>
    <div className="timeline">
      ...
      {!history.length && <div className="thehive-empty">Chưa có lịch sử.</div>}
    </div>
  </>);
}
