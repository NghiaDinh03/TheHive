...
export default function AdminPage() {
  ...
  const deleteOrgMutation = useMutation({ mutationFn: (id: string) => stepUpFetch(`/api/v1/admin/organisations/${encodeURIComponent(id)}`, { method: 'DELETE' }, `Xóa Organisation ${id}`), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['admin-organisations'] }); reportSuccess('Organisation deleted.'); }, onError: reportError });
  ...
  const deleteProfileMutation = useMutation({ mutationFn: (id: string) => stepUpFetch(`/api/v1/admin/profiles/${encodeURIComponent(id)}`, { method: 'DELETE' }, `Xóa Profile ${id}`), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['admin-profiles'] }); reportSuccess('Profile deleted.'); }, onError: reportError });
  const deleteUserMutation = useMutation({ mutationFn: (login: string) => stepUpFetch(`/api/v1/admin/users/${encodeURIComponent(login)}`, { method: 'DELETE' }, `Xóa User ${login}`), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['admin-users'] }); reportSuccess('User deleted.'); }, onError: reportError });
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 backdrop-blur-md z-[1100]">
          <div className="bg-gray-900 border border-red-500/50 rounded-xl shadow-2xl w-full max-w-[480px] overflow-hidden">
            <div className="bg-red-600 px-5 py-4 flex items-center gap-3">
              ...
              <h3 className="text-white font-medium text-lg m-0">Xác thực bảo mật (Step-Up 2FA)</h3>
            </div>
            <div className="p-6">
              <div className="flex flex-col items-center mb-6">
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-2 rounded-lg text-sm font-medium mb-4 w-full text-center">
                  Hành động này yêu cầu quyền quản trị cao
                </div>
                {stepUpPending.actionDesc && (
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 w-full text-center shadow-inner">
                    <span className="text-gray-400 text-xs uppercase tracking-wider block mb-1">Hành động đang thực hiện</span>
                    <strong className="text-white text-base">{stepUpPending.actionDesc}</strong>
                  </div>
                )}
              </div>
              <div className="flex flex-col items-center">
                <p className="text-gray-400 text-sm mb-3">Vui lòng nhập mã 2FA từ ứng dụng Authenticator để xác nhận.</p>
                <input
                  type="text"
            ...
            <div className="bg-gray-800/80 px-6 py-4 border-t border-gray-700 flex justify-end gap-3">
              <button className="px-5 py-2.5 text-sm font-medium text-gray-300 hover:text-white bg-gray-700 hover:bg-gray-600 rounded-lg transition-colors" onClick={stepUpPending.cancel}>Hủy</button>
              <button 
              ...
              {paginatedUsers.map((user) => (
                <tr key={user.login} className={`hover:bg-gray-800/30 transition-colors cursor-pointer ${selected?.login === user.login && !editMode ? 'bg-gray-800/50' : ''}`} onClick={() => !editMode && setSelected(user)}>
                  <td className="px-5 py-3 font-medium text-white whitespace-nowrap">
                    ...
                    {user.force_2fa && <div className="mt-1"><span className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-500/20 text-red-400 border border-red-500/30">Bắt buộc 2FA</span></div>}
                  </td>
            ...
            <div className="p-4 flex flex-col gap-4 overflow-y-auto max-h-[70vh]">
              ...
              {selected && (
                <label className="flex items-center gap-2 cursor-pointer p-3 border border-red-500/30 bg-red-500/10 rounded-md">
                  ...
                  <span className="text-red-400 text-sm font-medium">Bắt buộc bật 2FA (Bảo mật)</span>
                </label>
              )}
...
function ProfileAdmin({ values, canManage, loading, onCreate, onUpdate, onDelete }: { values: Profile[]; canManage: boolean; loading: boolean; onCreate: (payload: Record<string, unknown>) => void; onUpdate: (id: string, payload: Record<string, unknown>) => void; onDelete: (id: string) => void }) {
  ...
  const groups = useMemo(() => {
    ...
    for (const p of allPermissions) {
      const group = PERMISSION_METADATA[p as Permission]?.group || 'Khác';
