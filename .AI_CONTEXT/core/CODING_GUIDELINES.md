...
|------|---------|-----------|
| `.AI_CONTEXT/MEMORY.md` | Append-only log | **NEVER delete entries. Only append.** |
| `.AI_CONTEXT/STRUCTURE.md` | Project structure map | **Update immediately when structure changes.** |
| `.AI_CONTEXT/CODING_GUIDE.md` | This file — all rules | **Must run Self-Debate Protocol before adding rules.** |
| `.AI_CONTEXT/context.md` | Product/architecture context | Stable; update only for major architectural changes. |
| `.AI_CONTEXT/plan.md` | Control plan | Clean plan; completed evidence goes to plan_done.md. |
| `.AI_CONTEXT/plan_done.md` | Completed evidence | Only mark done with code/test evidence. |
| `.AI_CONTEXT/plan_unfinish.md` | Unfinished backlog | Only unchecked/partially proven tasks. |

### 2.4 Read Before Write

- **Always read relevant files before making changes.** Never edit blindly.
- Read `.AI_CONTEXT/context.md` and `.AI_CONTEXT/plan.md` at the start of every session.
- Check `.AI_CONTEXT/STRUCTURE.md` to find where things live.
...
- Preserve TheHive 4 AdminLTE skin-blue theme tokens:
  ...
  - Body bg: `#ecf0f5`
  - Text: `#333`
- **Severity Color Palette (Synchronized - 2026-05-21)**:
  - **Low (Level 0)**: Blue (Xanh dương)
    - Tailwind: `bg-blue-500/20 text-blue-400 border-blue-500/30` / CSS: `.severity-0` (`#2563eb`)
  - **Medium (Level 1)**: Yellow (Vàng)
    - Tailwind: `bg-yellow-500/20 text-yellow-400 border-yellow-500/30` / CSS: `.severity-1` (`#eab308`)
  - **High (Level 2)**: Orange (Cam)
    - Tailwind: `bg-orange-500/20 text-orange-400 border-orange-500/30` / CSS: `.severity-2` (`#f97316`)
  - **Critical (Level 3 & 4)**: Red (Đỏ)
    - Tailwind: `bg-red-500/20 text-red-400 border-red-500/40` or `bg-red-600` / CSS: `.severity-3`, `.severity-4` (`#ef4444`)
